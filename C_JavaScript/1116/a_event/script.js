...
// : 예) 사용자가 버튼을 클릭, 웹 페이지가 로드, input필드에 입력하는 것 등

//! 2. 이벤트 핸들링
// : 특정 이벤트에 반응해서 특정 동작을 실행하는 것을 의미
// : 이벤트 핸들러(이벤트 리스너)는 특정 이벤트가 발생했을 때 호출되는 함수

// 예) 사용자가 버튼을 클릭했을 때 alert창이 실행되도록 작성
...
// - btn.onfocus: 버튼이 포커스되었을 때 발생
//    (tab 사용 시 포커스)
// - btn.onblur: 포커스가 해제되었을 때 실행 (탭으로 보더라인 짙어졌을때 tab을 눌러 다른곳으로 가면 실행)
...
// -btn.onmouseover, btn.onmouseout: 각각 마우스 포인터가 버튼에서 벗어났을 때 실행
...
function bgChange() {
  ...
  function random(number) {
    ...
    // Math.random(): 0 이상 1미만의 부동 소수점 난수(랜덤 숫자)를 반환
