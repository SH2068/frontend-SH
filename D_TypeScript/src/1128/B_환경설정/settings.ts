...
: TypeScripts는 JS의 슈퍼셋이므로 'Node.js' 환경에서 동작
...
: 설치 완료 여부 확인
  ...
  : npm 버전 변경
    ...
    - 특정 버전으로 변경
    npm install -g npm@버전번호(10.2.3)

? Windows에서 npc 실행이 되지 않을 경우
  ...
  : 사용자 변수에서 Path 설정
    '사용자 이름'에 대한 사용자 변수
    > C드라이브 > Users(사용자)
    > 자신의 컴퓨터 이름(User) > AppData > Roaming > npm의 경로를 복사
...
: 전역 설치
  ...
  dependencis: 프로젝트 실행에 필요한 패키지들의 목록

? tsconfig.json 생성
: 타입스크립트 프로젝트의 설정을 저장하는 파일
: 타입스크립트 컴파일러가 프로젝트를 컴파일 하는 방법을 지정
